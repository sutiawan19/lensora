...
import { 
  Camera, Search, Bell, Calendar, MessageSquare, 
  MapPin, ArrowLeft, Send, Paperclip, CheckCircle2, Pin, TrendingUp
} from "lucide-react";
...
const CONTACTS_INITIAL = [
  {
    ...
    lastMessage: "Halo! Saya mau lebih banyak ambil spot di dekat amphiteater.",
    time: "10:40 AM",
    unread: 2,
  ...
  },
  {
    ...
    lastMessage: "Terima kasih mas, ditunggu hasil editannya ya!",
    time: "Kemarin",
    unread: 0,
    statusBadge: "Editing Process",
    pinned: false,
    online: false,
    refId: "ORD-002"
  },
  {
    ...
    lastMessage: "Mas, fotonya sudah saya unduh, mantap hasilnya!",
    time: "Senin",
    unread: 0,
...
const MOCK_CHATS: Record<string, any[]> = {
  c1: [
    { id: "m1", type: "system", text: "Booking has been confirmed. Package: Pre-Wedding Basic", time: "Mon, 10:00 AM" },
    { id: "m2", type: "vendor", text: "Halo kak Budi! Terima kasih sudah memesan sesi Pre-Wedding Basic. Lokasi di Taman Hutan Raya, Bandung ya?", time: "Mon, 10:15 AM" },
    { id: "m3", type: "user", text: "Halo! Betul mas. Mohon siapkan konsep casual outdoor. Referensi foto sudah dikirim via WhatsApp sebelumnya.", time: "Mon, 10:40 AM" },
    { id: "m4", type: "user", text: "Ini kira-kira referensi posenya biar lebih jelas.", img: "https://images.unsplash.com/photo-1523050854058-8df90110c9f1?q=80&w=400&auto=format&fit=crop", time: "Mon, 10:41 AM" },
  ...
  c2: [
    { id: "m1", type: "system", text: "Booking has been confirmed. Package: Graduation Premium", time: "Tue, 09:00 AM" },
    { id: "m2", type: "user", text: "Halo mas, untuk wisuda besok di Universitas Padjadjaran, Jatinangor. Foto bareng keluarga inti saja ya.", time: "Tue, 09:15 AM" },
    { id: "m3", type: "user", text: "Bawa lensa wide ya mas biar muat semua di frame.", time: "Tue, 09:16 AM" },
    { id: "m4", type: "vendor", text: "Siap kak Siti! Saya pasti bawa lensa wide. Ketemu jam 10 pagi ya di venue.", time: "Tue, 10:00 AM" },
    { id: "m5", type: "system", text: "Sesi telah selesai, masuk ke tahap editing.", time: "Kemarin, 14:00 PM" },
    { id: "m6", type: "user", text: "Terima kasih mas, ditunggu hasil editannya ya!", time: "Kemarin, 15:30 PM" }
  ],
  c3: [
    { id: "m1", type: "system", text: "Booking has been confirmed. Package: Personal Portrait", time: "Sun, 14:00 PM" },
    { id: "m2", type: "vendor", text: "Halo pak Agus, untuk Personal Portrait di Studio Lensora nanti sore, ada referensi spesifik?", time: "Sun, 14:15 PM" },
    { id: "m3", type: "user", text: "Halo mas, ini untuk foto profile LinkedIn dan CV. Jadi butuh yang profesional dengan background polos.", time: "Sun, 14:30 PM" },
    { id: "m4", type: "vendor", text: "Baik pak, kita siapkan background abu-abu dan putih ya.", time: "Sun, 14:45 PM" },
    { id: "m5", type: "system", text: "Photoshoot session completed.", time: "Senin, 17:00 PM" },
    { id: "m6", type: "user", text: "Mas, fotonya sudah saya unduh, mantap hasilnya!", time: "Senin, 18:00 PM" }
  ]
};

const QUICK_REPLIES = [
  "Siap kak, kita ketemu di lokasi ya.",
  "Boleh kirim referensi posenya?",
  "Untuk pakaian bebas asal senada.",
...
export default function PhotographerMessages() {
  ...
  const [messages, setMessages] = useState(MOCK_CHATS["c1"]);
  ...
  const handleSendMessage = () => {
    ...
    // Update contact list's last message
    setContacts(prevContacts => prevContacts.map(c => {
      if (c.id === activeContactId) {
        return { ...c, lastMessage: newMsgText, time: "Baru saja" };
      }
      return c;
    }));
  ...
  return (
    <div className="flex-1 flex flex-col md:flex-row overflow-hidden pb-[60px] md:pb-0 h-[100dvh]">
      ...
      <div className={`w-full md:w-80 xl:w-96 bg-white border-r border-border flex flex-col shrink-0 ${!showListOnMobile ? 'hidden md:flex' : 'flex'}`}>
         ...
         <div className="h-16 px-4 md:px-6 flex items-center justify-between border-b border-border shrink-0">
            <h2 className="font-extrabold text-xl text-foreground">Inbox Klien</h2>
         </div>
         ...
         <div className="p-4 border-b border-border shrink-0">
            <div className="relative">
               ...
               <input 
                 ...
                 className="w-full pl-9 pr-3 py-2 bg-surface-2 border border-border rounded-xl text-sm focus:outline-none focus:border-primary transition-colors" 
               />
            </div>
         </div>
               ...
               >
                  ...
                  <div className="flex gap-3">
                     <div className="relative shrink-0">
                        <img src={contact.avatar} alt={contact.clientName} className="w-12 h-12 rounded-full object-cover" />
                     ...
                     <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start mb-0.5 pr-4">
                           <h3 className="font-extrabold text-sm text-foreground truncate">{contact.clientName}</h3>
                        ...
                        <div className="flex items-center justify-between mt-2">
                           <span className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded uppercase tracking-wider ${contact.statusBadge.includes('Upcoming') ? 'bg-amber/10 text-amber' : contact.statusBadge.includes('Pending') ? 'bg-blue-100 text-blue-600' : 'bg-surface text-text-muted'}`}>
                              {contact.statusBadge}
                           </span>
      ...
      <div className={`flex-1 flex flex-col bg-surface min-w-0 ${showListOnMobile ? 'hidden md:flex' : 'flex'}`}>
         ...
         <header className="h-16 px-4 md:px-6 bg-white border-b border-border flex items-center justify-between shrink-0 shadow-sm z-10 relative">
            ...
            <div>
               <Link href={`/fotografer/pemesanan?orderId=${activeContact.refId}`} className="inline-block px-4 py-1.5 bg-surface-2 hover:bg-border text-foreground text-xs font-bold rounded-lg transition-colors border border-border">
                  Detail Pesanan
               </Link>
            </div>
         </header>
         ...
         <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
            ...
            {messages.map(msg => {
               ...
               // For vendor perspective, "vendor" is the sender (isUser = true in UI logic), "user" is the client (isUser = false)
               const isSender = msg.type === "vendor";
               
               return (
                  <div key={msg.id} className={`flex w-full ${isSender ? "justify-end" : "justify-start"}`}>
                     <div className={`flex max-w-[85%] md:max-w-[70%] ${isSender ? "flex-row-reverse" : "flex-row"} gap-2 md:gap-3`}>
                        {/* Avatar */}
                        {!isSender && (
                        ...
                        <div className={`flex flex-col ${isSender ? "items-end" : "items-start"}`}>
                           ...
                           <div className={`p-3 md:p-4 rounded-2xl ${
                              ...
                              {msg.text && <p className={`text-sm font-medium leading-relaxed ${isSender ? 'text-white' : 'text-foreground'}`}>{msg.text}</p>}
                           </div>
