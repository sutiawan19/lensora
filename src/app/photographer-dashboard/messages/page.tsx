...
const CONTACTS_INITIAL = [
  ...
  {
    id: "c2",
    clientName: "Anita & Johan",
    avatar: "https://i.pravatar.cc/150?img=5",
    lastMessage: "Terima kasih atas fotonya mas, hasilnya keren banget!",
    time: "Kemarin",
    unread: 0,
  ...
  {
    id: "c3",
    clientName: "Rizky (Event)",
    avatar: "https://i.pravatar.cc/150?img=33",
    lastMessage: "Mas, untuk acara besok butuh berapa lampu lighting?",
    time: "Senin",
    unread: 0,
    ...
    <main className="h-[100dvh] bg-surface-2 flex flex-col md:flex-row overflow-hidden pb-[60px] md:pb-0">
      ...
      <aside className="w-20 lg:w-64 bg-white border-r border-border flex flex-col shrink-0 hidden md:flex">
        ...
        <div className="p-4 flex-1">
          <p className="hidden lg:block text-xs font-bold text-text-muted uppercase tracking-wider mb-4 px-3">Photographer Panel</p>
          <nav className="space-y-1">
             <Link href="/photographer-dashboard" className="flex items-center justify-center lg:justify-start gap-3 px-3 py-2 text-text-muted hover:bg-surface-2 hover:text-foreground font-semibold rounded-xl transition-colors" title="Statistik & Overview">
               <TrendingUp className="w-5 h-5 shrink-0" /> <span className="hidden lg:block">Statistik & Overview</span>
             </Link>
             <Link href="/photographer-dashboard/sessions" className="flex items-center justify-center lg:justify-start gap-3 px-3 py-2 text-text-muted hover:bg-surface-2 hover:text-foreground font-semibold rounded-xl transition-colors" title="Jadwal Sesi">
               <Calendar className="w-5 h-5 shrink-0" /> <span className="hidden lg:block">Jadwal Sesi</span>
             </Link>
             <Link href="/photographer-dashboard/messages" className="flex items-center justify-center lg:justify-start gap-3 px-3 py-2 bg-primary-light text-primary font-bold rounded-xl transition-colors" title="Pesan">
      ...
      <div className={`w-full md:w-80 xl:w-96 bg-white border-r border-border flex flex-col shrink-0 ${!showListOnMobile ? 'hidden md:flex' : 'flex'}`}>
         ...
         <div className="h-16 px-4 md:px-6 flex items-center justify-between border-b border-border shrink-0">
            ...
            <button onClick={() => alert("Notifikasi belum tersedia")} className="p-2 bg-surface-2 rounded-full text-text-muted hidden md:block"><Bell className="w-4 h-4" /></button>
         </div>
      ...
      <div className={`flex-1 flex flex-col bg-surface min-w-0 ${showListOnMobile ? 'hidden md:flex' : 'flex'}`}>
         ...
         <header className="h-16 px-4 md:px-6 bg-white border-b border-border flex items-center justify-between shrink-0 shadow-sm z-10 relative">
            ...
            <div>
               <button onClick={() => alert("Detail pesanan akan segera hadir!")} className="px-4 py-1.5 bg-surface-2 hover:bg-border text-foreground text-xs font-bold rounded-lg transition-colors border border-border">
                  Detail Pesanan
               </button>
            </div>
         </header>
